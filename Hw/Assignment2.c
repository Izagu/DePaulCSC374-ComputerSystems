...
int 	      	isZero(float f)
{
	...
	//  Your code here
	//return (!(u & EVERYTHING_BUT_SIGN_MASK == 0));
...
int		getSign(float f)
{
	...
	//  Your code here
	//return (!(u & SIGN_MASK == 0))
	if (u & SIGN_MASK == 0) {
		return 1;
	}

	return(-1 /* Perhaps change this */);
}
...
{
	unsigned int	u = *(unsigned int*)& f;
	unsigned int 	i = EXPONENT_MASK & u; /* Perhaps change this */
	
	//  Your code here
	// IF GET POWER 2 RETURNS DENORMALIZED POWER OF 2 DO NOT GET HIDDEN BIT
	u = u >> EXPONENT_SHIFT; //gets rid of exponent
	int num = u - INFINITE_POWER_OF_2 + 1;
	if (DENORMALIZED_POWER_OF_2 > num) {
		return DENORMALIZED_POWER_OF_2;
		}
	
	return(num /* Perhaps change this */);
}
...
unsigned int	getMantissa(float f)
{
	...
	num = mantissa | MANTISSA_HIDDEN_BIT;
	//  Your code here
	//MAKE MANTISSA SHIFT IT TO THE RIGHT CLEAR ALL THE MANTISSA BITS
	//
